  ...
  return (
    <>
      <div className="relative z-10 ml-6 md:ml-4">
        <Menu>
          <Menu.Button className="relative w-8 h-8 group">
            <Icon
            ...
            <Menu.Items className="absolute top-full -left-2 w-[13.75rem] mt-1 p-3 bg-n-1 rounded-[1.25rem] shadow-[0_0_1rem_0.25rem_rgba(0,0,0,0.04),0_2rem_2rem_-1rem_rgba(0,0,0,0.1)] outline-none lg:left-auto lg:-right-6 dark:bg-n-7 dark:border dark:border-n-5">
              <div className="space-y-2">
                {menu.map((item, index) => (
                  <Menu.Item key={index}>
                    <button
                      className="flex items-center w-full h-12 px-3 font-semibold transition-colors rounded-lg group base1 text-n-4 hover:bg-n-2 hover:text-n-7 dark:hover:bg-n-6 dark:hover:text-n-1"
                    ...
                    >
                      <Icon
                        ...
                        name={item.id === "0" ? (favorite ? "star-fill" : item.icon) : item.icon}
                      />
                      {item.title}
                    </button>
                  </Menu.Item>
