...
const CustomPre = ({ children }) => {
  ...
  return (
    <div className="flex flex-col mb-4 ">
      <div className="flex items-center justify-between w-full p-1 px-4 text-white rounded-t-lg bg-zinc-800">
        <div>{language.charAt(0).toUpperCase() + language.slice(1)}</div>
        <button
          onClick={handleCopyClick}
          className="flex items-center gap-2 px-2 py-1 rounded hover:bg-zinc-600 focus:outline-none"
        >
          <FaCopy />
...
const CustomCodeBlock = ({ inline = false, className = "", children }) => {
...
const CustomLink = ({ children, href }) => {
  return (
    <a className="overflow-hidden link" href={href} target="_blank" rel="noopener noreferrer">
      {children}
    </a>
  );
};

const isValidCustomCodeBlock = (element) =>
  React.isValidElement(element) && element.type === CustomCodeBlock;

const extractLanguageName = (languageString) => {
